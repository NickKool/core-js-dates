...
function getTime(date) {
  ...
  const hh = dat.getHours().toString().padStart(2, '0');
...
function getNextFriday(date) {
  ...
  const friday = ((5 - next.getDay() + 6) % 7) + 1;
  next.setDate(next.getDate() + friday);
  return next;
}
...
function formatDate(date) {
  const dat = new Date(date);
  return dat.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let sum = 0;
  for (let i = 1; i <= x; i += 1) {
    const dat = new Date(year, month - 1, i);
